                ...
                <tr data-id="<?= $p['id'] ?>"
                    ...
                    <td class="fw-semibold">
                        <?php if(!empty($p['image_url'])): ?>
                            <img src="<?= sanitize($p['image_url']) ?>" alt="" style="width:28px;height:28px;object-fit:cover;border-radius:4px;margin-right:8px;vertical-align:middle;">
                        <?php endif; ?>
                        <?= sanitize($p['nom']) ?>
                    </td>
                    <td><span class="cat-pill"><?= sanitize($p['categorie']) ?></span></td>
                    <td class="price-cell"><?= formatPrice((float)$p['prix']) ?></td>
                    <td><span style="font-weight:700;"><?= (int)($p['stock'] ?? 0) ?></span></td>
                    <td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <label class="field-label" style="color:var(--text-2)">Nom du produit <span style="color:var(--danger);">*</span></label>
                        <input type="text" id="editNom" class="pos-input" placeholder="Ex: Café noir, Sandwich thon…" required>
                    </div>
                    <div class="col-md-6">
                        <label class="field-label" style="color:var(--text-2)">Prix (HTG) <span style="color:var(--danger);">*</span></label>
                        <input type="number" id="editPrix" class="pos-input" placeholder="0.00" step="0.01" min="0" required>
                    </div>
                    <div class="col-md-6">
                        <label class="field-label" style="color:var(--text-2)">Catégorie</label>
                    ...
                    <div class="col-md-6">
                        <label class="field-label" style="color:var(--text-2)">Code-barres (Optionnel)</label>
                        <input type="text" id="editBarcode" class="pos-input" placeholder="123456789">
                    </div>
                    <div class="col-md-4">
                        <label class="field-label" style="color:var(--text-2)">Stock</label>
                        <input type="number" id="editStock" class="pos-input" placeholder="0" min="0" value="0">
                    </div>
                    <div class="col-md-8">
                        <label class="field-label" style="color:var(--text-2)">Statut</label>
                    ...
                    <div class="col-12">
                        <label class="field-label" style="color:var(--text-2)">Image produit (Optionnel)</label>
                        <input type="file" id="editImage" class="pos-input" accept="image/*" style="padding:6px 10px;">
                    </div>
                </div>
...
function ouvrirModalEdit(row) {
    ...
    document.getElementById('editImage').value     = ''; // Can't preset file input
...
async function sauvegarder() {
    ...
    if (image) {
        ...
        const body = JSON.stringify({ nom, prix, categorie: cat, stock, barcode, actif: parseInt(actif) });
