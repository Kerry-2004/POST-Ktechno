...
<div class="controls">
    ...
    <button class="btn-p" onclick="openEmailModal()" style="background:#16a34a;">📧 Envoyer par email</button>
    <button class="btn-c" onclick="window.close()">✕ Fermer</button>
</div>

<!-- Modal email -->
<div id="emailModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:none;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:16px;padding:28px 24px;max-width:380px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,.3);">
        <h6 style="font-weight:800;margin-bottom:16px;color:#1a1740;">📧 Envoyer le ticket par email</h6>
        <input type="email" id="emailInput" placeholder="adresse@email.com"
               style="width:100%;padding:10px 14px;border:1.5px solid #e8e6fb;border-radius:10px;font-size:.9rem;margin-bottom:12px;">
        <div id="emailMsg" style="font-size:.82rem;margin-bottom:10px;display:none;"></div>
        <div style="display:flex;gap:10px;">
            <button onclick="sendEmail()" style="flex:1;background:#453dde;color:#fff;border:none;border-radius:10px;padding:10px;font-weight:700;cursor:pointer;">Envoyer</button>
            <button onclick="closeEmailModal()" style="flex:1;background:#f1f0ff;color:#453dde;border:none;border-radius:10px;padding:10px;font-weight:700;cursor:pointer;">Annuler</button>
        </div>
    </div>
</div>
...
<div class="ticket">
    ...
    <?php if ((float)($commande['discount'] ?? 0) > 0): ?>
    <div class="subtotal" style="color:#7c3aed;">
        <span>Remise</span>
        <span>-<?= number_format((float)$commande['discount'], 0, ',', ' ') ?> HTG</span>
    </div>
    <?php endif; ?>

    <?php
    $pm = $commande['payment_method'] ?? 'especes';
    $pmLabel = ['especes'=>'Espèces','credit'=>'Crédit','mobile'=>'Mobile money'];
    $paid = (float)($commande['amount_paid'] ?? 0);
    $rendu = $pm === 'especes' && $paid > 0 ? max(0, $paid - (float)$commande['total_amount']) : 0;
    ?>
    <div class="subtotal">
        <span>Paiement</span><span><?= $pmLabel[$pm] ?? ucfirst($pm) ?></span>
    </div>
    <?php if ($pm === 'especes' && $paid > 0): ?>
    <div class="subtotal">
        <span>Reçu</span><span><?= number_format($paid, 0, ',', ' ') ?> HTG</span>
    </div>
    <?php if ($rendu > 0): ?>
    <div class="subtotal" style="color:#16a34a;">
        <span>Monnaie rendue</span><span><?= number_format($rendu, 0, ',', ' ') ?> HTG</span>
    </div>
    <?php endif; endif; ?>
...
<script>
    ...
    async function sendEmail() {
        ...
        if (!email) { msgEl.textContent = 'Veuillez saisir un email.'; msgEl.style.display=''; msgEl.style.color='#dc2626'; return; }
        msgEl.textContent = 'Envoi en cours…'; msgEl.style.display=''; msgEl.style.color='#6b6897';
        try {
            const res  = await fetch('index.php?page=commande&action=sendEmail&id=<?= $commande['id'] ?>', {
                method:'POST', headers:{'Content-Type':'application/json'},
                body: JSON.stringify({email})
            });
            const data = await res.json();
            if (data.success) {
                msgEl.textContent = '✅ Email envoyé avec succès !';
